...
class Controller(object):
    ...
    def control(self, linear_velocity, angular_velocity, current_velocity):
        ...
        tgt_throttle = 0.3*(linear_velocity - current_velocity)
        throttle = tgt_throttle
        throttle = self.throttlePID.step(throttle, TIME_STEP) 
#        if current_velocity > linear_velocity:
#            throttle = min(0.0, throttle)
#        if current_velocity < linear_velocity:
#            throttle = max(0.0, throttle)
        throttle = self.throttleLPF.filt(throttle)
        # negative throttle equals braking.  Braking needs to be converted to N*m with max in range of 3000-5000Nm
        brake = 0.
        if throttle < 0.0:
#            brake = throttle * MAX_BRAKE_PRESSURE / self.decel_limit # convert throttle to brake pressure (Nm), e.g +120Nm:+3500Nm
            brake = -throttle * MAX_BRAKE_PRESSURE # convert throttle to brake pressure (Nm), e.g +120Nm:+3500Nm
            throttle = 0.0;

        # maintain brake pressure when target speed is zero and vehicle is stopped
